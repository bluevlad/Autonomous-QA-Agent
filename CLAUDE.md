...
동작 흐름 (8단계): Health Check → Playwright 테스트 → 로그 저장 → 개선 제안 분석 → Slack 알림 → GitHub Issues → 이슈 자동 close → 로그 정리
...
- 8단계 파이프라인: Health Check → 테스트 → 로그 → 개선 제안 → Slack → Issues → 자동 close → 정리
