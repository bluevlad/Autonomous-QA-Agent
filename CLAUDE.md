...
│   ├── dashboard-sender.ts # Dashboard API 전송 모듈
...
- PostgreSQL에 QA 실행 결과를 장기 보관 (tsvector 전문검색)
- macOS OrbStack에 Docker Compose로 배포 (API: 9095, Web: 4095)
- QA Agent → Dashboard API(`/api/ingest`)로 실행 결과 자동 전송
- 3개 페이지: Dashboard(프로젝트 카드 + 최근 실행), RunDetail(상세), ProjectTimeline(30일 히트맵)
