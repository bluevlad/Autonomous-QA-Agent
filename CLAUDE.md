...
```
Autonomous-QA-Agent/
├── playwright.config.ts    # 통합 Playwright 설정 (6개 프로젝트)
...
│   ├── index.ts            # 엔트리포인트 (8단계 파이프라인)
│   ├── types.ts            # 공통 타입 정의 (FailureDetail, WBS, Priority 등)
│   ├── config.ts           # 6개 프로젝트 Health Check URL 매핑
│   ├── health-checker.ts   # Health Check 모듈 (fetch + AbortController)
│   ├── test-runner.ts      # Playwright 실행 모듈 (failureDetails 수집)
│   ├── dashboard-sender.ts # Dashboard API 전송 모듈 (재시도 포함)
│   ├── migrate-logs.ts     # 기존 JSON 로그 → DB 일괄 마이그레이션
│   ├── wbs-parser.ts       # WBS YAML 파서 (테스트-기능 매핑)
│   ├── issue-reporter.ts   # GitHub Issues (우선순위/WBS/자동close)
│   ├── improvement-advisor.ts  # 개선 제안 엔진 (5규칙 패턴 분석)
│   ├── slack-notifier.ts   # Slack Block Kit 알림 (우선순위/제안)
│   ├── logger.ts           # JSON 로그 저장/조회/정리
│   └── logs/               # 실행 로그 (gitignore)
├── projects/               # 테스트 대상 프로젝트들 (6개)
│   ├── hopenvision/        # 공무원 시험 채점 시스템
│   ├── AllergyInsight/     # 알러지 논문 검색 시스템
│   ├── EduFit/             # 학원/강사 평판 분석 통합 플랫폼
│   ├── StandUp/            # 업무보고 관리 자동화 Agent
│   ├── NewsLetterPlatform/ # 멀티테넌트 뉴스레터 플랫폼
│   ├── unmong-main/        # 운몽시스템즈 통합 포털
│   └── archive/            # 폐기된 프로젝트 (TeacherHub, HealthPulse 등)
├── dashboard/              # QA Dashboard (Phase 1 MVP)
│   ├── docker-compose.yml  # Docker 배포 (OrbStack)
...
npm run test:edufit             # EduFit만 테스트
...
npm run migrate:logs        # 기존 JSON 로그 → Dashboard DB 마이그레이션 (미전송만)
npm run migrate:logs -- --all  # 전체 재전송 (강제)
```

동작 흐름 (8단계): Health Check → Playwright 테스트 → 로그 저장 → Dashboard 전송 → 개선 제안 분석 → Slack 알림 → GitHub Issues + 자동 close → 로그 정리
...
- `DASHBOARD_API_URL`: Dashboard API URL (설정 시 전송 활성화)
...
- 8단계 파이프라인: Health Check → 테스트 → 로그 → Dashboard → 개선 제안 → Slack → Issues → 자동 close → 정리
- 스케줄러 실행 시 `SCHEDULER_MODE=true`로 Playwright Slack reporter 비활성화 (이중 알림 방지)
- 스케줄러 전용 Slack 알림: 서비스 상태 + 테스트 요약 + 응답시간 + 개선 제안
- 실행 로그: `scheduler/logs/run-YYYYMMDD-HHmmss.json` (30일 보관)

### 6. QA Dashboard
- `dashboard/` 하위에 Backend(FastAPI) + Frontend(React + Ant Design) 구성
- PostgreSQL 7개 테이블: qa_runs, qa_health_results, qa_endpoint_results, qa_test_results, qa_failure_details, qa_suggestions, qa_issue_results
- macOS OrbStack에 Docker Compose로 배포 (API: 9095, Web: 4095)
- QA Agent → Dashboard API(`/api/ingest`)로 실행 결과 자동 전송 (실패 시 5초 후 재시도)
- 로컬 실행(Windows) → 내부 네트워크로 직접 전송, GitHub Actions → Artifact만 저장
- `migrate-logs.ts`로 기존 JSON 로그를 DB에 일괄 마이그레이션 (멱등성 보장)
- 데이터 보관 정책: 상세 90일, raw_json 60일, 집계 1년 (`/api/cleanup` API)

### 7. WBS 매핑
- 각 프로젝트의 `wbs.yml`에 기능 요구사항과 테스트 파일 매핑 정의
- 테스트 실패 시 영향받는 기능(WBS 항목)을 이슈 본문에 자동 표시
- WBS 작성 가이드: `docs/WBS_GUIDE.md`
...
- 실패율 10~30% → P2 (보통)
- 그 외 → P3 (낮음)
- GitHub 이슈에 P0~P3 라벨 자동 부여, Slack에 우선순위 이모지 표시

### 9. 개선 제안 엔진
- `scheduler/improvement-advisor.ts`가 과거 로그(최대 30개)를 분석
- 5가지 규칙: 반복 실패, 응답시간 저하, 커버리지 부족, 인프라 이상, 실패율 추세
- 개선 제안을 `[QA-Improvement]` 제목으로 GitHub Issues에 자동 등록 (중복 방지)

### 10. 이슈 자동 close
- 이전에 실패했으나 현재 정상인 프로젝트의 `qa-auto` 라벨 open 이슈를 자동 close
- 코멘트로 복구 확인 내용 기록 후 close (2단계 실행)
...
| EduFit | 학원/강사 평판 분석 통합 플랫폼 | http://localhost:4070 | bluevlad/EduFit |
