...
```
Autonomous-QA-Agent/
├── playwright.config.ts    # 통합 Playwright 설정 (7개 프로젝트)
...
│   ├── index.ts            # 엔트리포인트 (7단계 파이프라인)
│   ├── types.ts            # 공통 타입 정의 (FailureDetail, WBS, Priority 등)
│   ├── config.ts           # 7개 프로젝트 Health Check URL 매핑
...
│   ├── Autonomous-QA-Agent/ # QA Dashboard 자체 점검
...
동작 흐름 (7단계): Health Check → Playwright 테스트 → 로그 저장 → 개선 제안 분석 → Slack 알림 → GitHub Issues + 자동 close → 로그 정리
...
- 7단계 파이프라인: Health Check → 테스트 → 로그 → 개선 제안 → Slack → Issues → 자동 close → 정리
...
| Autonomous-QA-Agent | QA Dashboard 관제 시스템 | http://172.30.1.72:4095 | bluevlad/Autonomous-QA-Agent |
